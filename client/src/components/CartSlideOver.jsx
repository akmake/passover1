...
import { useTranslation } from 'react-i18next'; // 1. ייבוא ספריית התרגום

export default function CartSlideOver() {
  const { isCartOpen, closeCart, items, removeFromCart, updateQuantity } = useCartStore();
  const isAuthenticated = useAuthStore((state) => state.isAuthenticated);
  const { t, i18n } = useTranslation(); // 2. הפעלת פונקציית התרגום
  ...
  return (
    <Transition.Root show={isCartOpen} as={Fragment}>
      <Dialog as="div" className="relative z-50" onClose={closeCart}>
        <div className="fixed inset-0 overflow-hidden"><div className="absolute inset-0 overflow-hidden"><div className="pointer-events-none fixed inset-y-0 right-0 flex max-w-full pl-10"><Transition.Child as={Fragment} enter="transform transition ease-in-out duration-500 sm:duration-700" enterFrom="translate-x-full" enterTo="translate-x-0" leave="transform transition ease-in-out duration-500 sm:duration-700" leaveFrom="translate-x-0" leaveTo="translate-x-full"><Dialog.Panel className="pointer-events-auto w-screen max-w-md">
          <div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
            <div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
              <div className="flex items-start justify-between">
                {/* --- התחלת התיקונים --- */}
                <Dialog.Title className="text-lg font-medium text-gray-900">{t('cart.title')}</Dialog.Title>
                <div className="ml-3 flex h-7 items-center"><button type="button" className="relative -m-2 p-2 text-gray-400 hover:text-gray-500" onClick={closeCart}><span className="sr-only">{t('common.closePanel')}</span><X className="h-6 w-6" aria-hidden="true" /></button></div>
              </div>
                  ...
                  {items.length > 0 ?
                    (
                      <ul role="list" className="-my-6 divide-y divide-gray-200">
                        {items.map((item) => {
                          const displayName = item.name?.[currentLang] || item.name?.he;
                                    ...
                                    <div className="mt-1 text-sm text-gray-500">
                                      {item.userChoices.map(choice => (
                                        <div key={choice.category}>
                                          <strong>{choice.category}:</strong> {choice.selectedOptions.map(opt => (opt.name?.[currentLang] || opt.name)).join(', ')}
                                        </div>
                                      ))}
                                    </div>
                                  </div>
