...
const AdminPackageEditPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const packageData = { name, price, description, isActive, fixedItems: validFixedItems, choiceRules };
    ...
    return (
        <div>
            <div className="flex items-center mb-6"><Link to="/admin/packages"><ArrowRight/></Link><h1 className="text-3xl font-bold mr-4">עריכת חבילה: {name}</h1></div>
            <form onSubmit={handleSubmit} className="space-y-8">
                <div className="bg-white p-6 rounded-lg shadow">
                    <h2 className="text-xl font-semibold mb-4">פרטים בסיסיים</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div><label>שם החבילה</label><input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full mt-1 p-2 border rounded-md" /></div>
